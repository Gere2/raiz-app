...
export async function updateCustomerProfile(order: {
  ...
  try {
    ...
    if (existing.exists()) {
      ...
      const segment = calculateSegment(newTotalVisits, 0)
...
export async function getAllProfiles(): Promise<CustomerProfile[]> {
  if (!db) return []
  try {
...
export async function getProfilesBySegment(segment: string): Promise<CustomerProfile[]> {
  if (!db) return []
  try {
    const q = query(collection(db, "customer_profiles"), where("segment", "==", segment))
