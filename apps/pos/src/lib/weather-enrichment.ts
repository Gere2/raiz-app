/**
 * weather-enrichment.ts (POS)
 *
 * Captura condiciones meteorológicas reales en cada transacción.
 * Usa Open-Meteo API (gratis, sin API key, sin registro).
 *
 * Recibe coordenadas y calendario como parámetros para ser
 * agnóstico a la organización — sin hardcodes.
 *
 * Usage:
 *   import { fetchCurrentWeather, getUniversityCalendar } from "@/lib/weather-enrichment";
 *   const weather = await fetchCurrentWeather({ lat: 40.4168, lon: -3.7038 });
 *   const calendar = getUniversityCalendar(new Date(), calendarConfig);
 */
