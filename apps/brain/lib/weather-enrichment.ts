/**
 * weather-enrichment.ts (configurable version)
 *
 * Captura condiciones meteorológicas reales en cada transacción.
 * Usa Open-Meteo API (gratis, sin API key, sin registro).
 *
 * Ahora recibe coordenadas y calendario como parámetros para ser
 * agnóstico a la organización — no hay hardcodes.
 */
...
// ── Mapeo de códigos WMO a condiciones ──
...
export async function fetchCurrentWeather(location: LocationConfig): Promise<WeatherData> {
  ...
  try {
    const tz = encodeURIComponent(location.timezone || "Europe/Madrid");
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${location.lat}&longitude=${location.lon}&current=temperature_2m,relative_humidity_2m,apparent_temperature,precipitation,weather_code,wind_speed_10m&timezone=${tz}`;
...
// ── Fallback estacional si la API falla ──
...
// ══════════════════════════════════════
// CALENDARIO ACADÉMICO CONFIGURABLE
// ══════════════════════════════════════
...
export function getCalendarContext(
  ...
  const isHoliday = (d: Date) => holidays.has(dateStr(d));
  ...
  return {
    ...
    isLastWeekOfClasses,
    isPreHoliday: isHoliday(tomorrow) && !isHoliday(date),
    isPostHoliday: isHoliday(yesterday) && !isHoliday(date),
    campusActivity,
    season: getSeason(month),
  };
}
