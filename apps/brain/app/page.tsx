...
export default function BrainApp() {
  ...
  const fetchProducts = useCallback(async () => { if (!user || !orgId) return; try { const r = await authedFetch(user, "/api/pos/products"); const d = await r.json(); setProducts(d.products || []); setCategories(d.categories || []); setErrors(e => ({ ...e, products: "" })); } catch (e) { console.error(e); setErrors(ex => ({ ...ex, products: "Error cargando datos. Pulsa para reintentar." })); } }, [user, orgId]);
  const fetchInventory = useCallback(async () => { if (!user || !orgId) return; try { const r = await authedFetch(user, "/api/pos/inventory"); const d = await r.json(); setInventory(d.items || []); setErrors(e => ({ ...e, inventory: "" })); } catch (e) { console.error(e); setErrors(ex => ({ ...ex, inventory: "Error cargando datos. Pulsa para reintentar." })); } }, [user, orgId]);
